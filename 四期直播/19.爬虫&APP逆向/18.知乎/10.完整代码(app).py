...
def md5_encrypt(data_string):
    obj = md5()
    data_bytes = pad(data_string.encode("utf-8"), 8)
    obj.update(data_bytes)
    return obj.hexdigest()


def encrypt(md5_str):
    with open("10.v2.js", mode="r", encoding="utf-8") as f:
        js = f.read()
...
def get_zse_96(zse_93, url_string, version, hd, udid):
    data_string = f"{zse_93}+{url_string}+{version}+{hd}+{udid}"
...
def run():
    ...
    url_string = f"/api/v4/search_v3?gk_version=gz-gaokao&q={q}&t=general&lc_idx=0&correction=1&offset=0&advert_count=0&limit=20&is_real_time=0&show_all_topics=0&search_source=History&filter_fields=&city=&pin_flow=false&ruid=&recq=&raw_query="

    zse_96 = get_zse_96(zse_93, url_string, version, hd, udid)
    print(zse_96)
    resp = requests.get(
