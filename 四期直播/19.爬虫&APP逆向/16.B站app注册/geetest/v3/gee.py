...
from .utils import get_distance, get_slide_track, turn_img_back


FOLDER_PATH = os.path.join("geetest", 'v3')
os.environ["NODE_PATH"] = os.path.join(FOLDER_PATH, "node_modules")
...
def do_geetest(challenge, gt, proxy_dict=None):
    ...
    t = str(int(time.time() * 1000))
    # get_php_url = f"https://api.geetest.com/ajax.php?gt={gt}&challenge={challenge}&lang=zh-cn&pt=0&client_type=web&w="
    get_php_url = f"https://api.geetest.com/ajax.php?gt={gt}&challenge={challenge}&lang=zh-cn&pt=3&client_type=web_mobile&callback=geetest_{t}&w="
    ...
    get_php_url = f"https://api.geetest.com/get.php?is_next=true&type=slide3&gt={gt}&challenge={challenge}&lang=zh-cn&https=false&protocol=https%3A%2F%2F&offline=false&product=embed&api_server=api.geetest.com&isPC=true&autoReset=true&width=100%25&callback=geetest_{t}"
    get_php_resp = session.get(get_php_url)
    ...
    f = open(os.path.join(FOLDER_PATH, "finally.js"), mode="r", encoding="utf-8")
    js_source = f.read()
    f.close()
    ...
    return json.loads(finally_resp.text.strip(callback_string).strip("(").strip(")")),challenge
